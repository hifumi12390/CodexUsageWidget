...
try {
 $releaseDir = 'dist/v1.4.1-win-x64'
 ...
 if ($Test) {
  $testOutput = Join-Path $PSScriptRoot 'tests/release-1.4.1'
 ...
 Compress-Archive -Path "$releaseDir/*" -DestinationPath dist/CodexUsageWidget-1.4.1-win-x64.zip -Force
 Get-FileHash (Join-Path $releaseDir 'CodexUsageWidget.exe'),dist/CodexUsageWidget-1.4.1-win-x64.zip -Algorithm SHA256 | Format-List
